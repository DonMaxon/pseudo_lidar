"""
save_pseudo_lidar_nuscenes_v12_4.py

v12.4 = v12 с исправлением ошибки #4:
  Bug 4 (радиус применения RANSAC): коррекция применялась ко всем точкам,
  включая дальние (>50м) через экстраполяцию плоскости.
  Fix: коррекция применяется только к точкам в радиусе max_radius=30м.
...
Запуск:
    python scripts/v12/save_pseudo_lidar_nuscenes_v12.py \\
        ...
        --version v1.0-mini \\
        --output_dir ./output/pseudo_lidar_nuscenes_v12_4
"""
...
def ransac_ground_plane_correction(points: np.ndarray,
    ...
    """
    Корректирует Z-координаты точек (только в радиусе max_radius) по наклону плоскости земли.
    ...
    Алгоритм:
      ...
      4. Для каждой точки в радиусе max_radius: z_corrected = z + (z_ref - predicted_z_ground(x,y)).
         Точки за пределами max_radius не корректируются (избегает экстраполяции).
    ...
    Args:
        ...
        max_radius: максимальный горизонтальный радиус (м) для применения коррекции
    """
    if len(points) == 0:
        return points
    ...
    try:
        ...
        mask_near = r_xy <= max_radius

        z_pred = ransac.predict(points[mask_near, :2]).astype(np.float32)
        corrected = points.copy()
        corrected[mask_near, 2] += (z_ref - z_pred)
        return corrected
...
def parse_args():
    p = argparse.ArgumentParser(description="v12.4: v12 + fix RANSAC radius (max_radius=30m)")
    p.add_argument("--dataroot",   default="/home/max/Phd/Phase1/Sparse4D/data/nuscenes")
    p.add_argument("--version",    default="v1.0-mini")
    p.add_argument("--output_dir", default="./output/pseudo_lidar_nuscenes_v12_4")
    ...
    p.add_argument("--ransac_max_radius",     type=float, default=30.0,
                   help="Макс. горизонтальный радиус для применения коррекции (м)")
    # Jump filter
