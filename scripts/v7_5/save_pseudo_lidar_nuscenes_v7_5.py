"""
save_pseudo_lidar_nuscenes_v7_5.py

v7.5 = v7 + SegFormer-B5 Cityscapes вместо SegFormer-B0 ADE20K.

Cityscapes специализирован на городских сценах (nuScenes — тоже городские),
что даёт более точные маски неба и растительности чем ADE20K.
...
Запуск:
    python scripts/v7_5/save_pseudo_lidar_nuscenes_v7_5.py \\
        --dataroot /home/max/Phd/Phase1/Sparse4D/data/nuscenes \\
        --version v1.0-mini \\
        --output_dir ./output/pseudo_lidar_nuscenes_v7_5
"""
...
# v7 ranges — не изменяем
X_RANGE = (-51.2, 51.2)
Y_RANGE = (-51.2, 51.2)
Z_RANGE = (-2.0, 1.5)
EGO_RADIUS = 2.5

# v7.5: SegFormer-B5 Cityscapes
_SEGFORMER_MODEL = "nvidia/segformer-b5-finetuned-cityscapes-1024-1024"
# Cityscapes: sky=10, vegetation=8
_CITYSCAPES_MASK_CLASSES = [10, 8]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="v7.5: v7 + SegFormer-B5 Cityscapes")
    p.add_argument("--dataroot", default="/home/max/Phd/Phase1/Sparse4D/data/nuscenes")
    p.add_argument("--version",  default="v1.0-mini")
    p.add_argument("--output_dir", default="./output/pseudo_lidar_nuscenes_v7_5")
...
def main() -> None:
    ...
    if not args.no_sky_mask or not args.no_veg_mask:
        # v7.5: SegFormer-B5 Cityscapes с явными class ID
        mask_classes = []
        if not args.no_sky_mask:
            mask_classes.append(10)   # Cityscapes sky
        if not args.no_veg_mask:
            mask_classes.append(8)    # Cityscapes vegetation
        sky_masker = SemanticMasker(
            model_id=_SEGFORMER_MODEL,
