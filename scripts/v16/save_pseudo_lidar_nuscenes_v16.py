"""
save_pseudo_lidar_nuscenes_v16.py

v16 = v15 + Bilateral Filter на карте глубины

Новая техника поверх v12:

5. Grid Penetration Filter (межкамерная проверка согласованности):
   После объединения точек всех 6 камер в глобальную систему координат,
   каждая точка проверяется: не находится ли она «за» поверхностью,
   которую видит хотя бы одна другая камера.

   Алгоритм:
     Для каждой точки p_global:
       Для каждой камеры C:
         1. Трансформировать p → систему координат C → получить Z_cam (глубина вдоль оптической оси)
         2. Спроецировать на изображение → пиксель (u, v)
         3. Считать эталонную глубину d_ref = depth_map_C[v, u]
         4. Если Z_cam > d_ref + threshold → точка «прокалывает» поверхность → удалить

   Удаляет артефакты depth bleeding: точки, предсказанные за реальными объектами
   из-за неточностей depth-модели на границах.

Новая техника поверх v15:

6. Bilateral Filter на карте глубины (перед Sobel):
   Edge-preserving smoothing: сглаживает шумную глубину на плоских поверхностях
   (дорога, стены, фасады) сохраняя резкие переходы между объектами.
   Использует RGB-изображение через sigmaColor (по значениям глубины) и
   sigmaSpace (пространственный Гауссиан).
   Результат — более "чистые" плоскости в облаке точек.

Запуск:
    python scripts/v16/save_pseudo_lidar_nuscenes_v16.py \\
        --dataroot /home/max/Phd/Phase1/Sparse4D/data/nuscenes \\
        --version v1.0-mini \\
        --output_dir ./output/pseudo_lidar_nuscenes_v16
"""
...
def apply_bilateral_filter(depth_map: np.ndarray,
    ...
    """
    Edge-preserving bilateral filter на карте глубины.

    Сглаживает шумную глубину на однородных поверхностях (дорога, стены),
    сохраняя резкие переходы между объектами на разных дистанциях.

    Args:
        depth_map:   (H, W) float32, глубина в метрах
        d:           диаметр окрестности пикселей
        sigma_depth: допуск по значению глубины (м); пиксели с разницей глубины
                     > sigma_depth вносят малый вклад в усреднение
        sigma_space: пространственный Гауссиан (пикселей)
    """
...
def grid_penetration_filter(points_global: np.ndarray,
    ...
    """
    Удаляет точки, которые находятся «за» поверхностью, видимой хотя бы одной камерой.

    Для каждой точки в глобальных координатах:
      1. Проецирует в систему координат каждой камеры.
      2. Сравнивает Z_cam (глубина вдоль оптической оси) с depth_map[v, u].
      3. Если Z_cam > depth_map[v, u] + threshold → точка за поверхностью → удалить.

    Args:
        points_global: (N, 3) float32 в глобальной системе координат
        cam_infos: список словарей {'depth_map': (H,W), 'K': (3,3), 'T_g2c': (4,4)}
        penetration_threshold: допуск (м); точка удаляется если она дальше
                               эталонной глубины более чем на threshold
    Returns:
        (M, 3) float32 — облако без «пробивающих» точек
    """
    ...
    for cam in cam_infos:
        depth_map = cam['depth_map']   # (H, W) float32
        K         = cam['K']           # (3, 3)
        T_g2c     = cam['T_g2c']       # (4, 4)
        H, W = depth_map.shape

        # Трансформация в систему камеры
        pts_cam = (T_g2c @ pts_h.T).T[:, :3]  # (N, 3)
...
def build_pseudo_lidar_global(cam_data_dict, estimator, back_proj, range_filter,
                               sky_masker=None,
                               sobel_threshold=2.0, sobel_dilation=3,
                               bilateral_d=9, bilateral_sigma_depth=1.0,
                               bilateral_sigma_space=9.0, no_bilateral=False):
    """
    Возвращает (points_global, cam_infos).
    cam_infos — список словарей с depth_map, K, T_g2c для каждой камеры.
    """
    all_points = []
    cam_infos  = []
    ...
    for cam_name, cam in cam_data_dict.items():
        ...
        # Сохраняем данные камеры для penetration filter
        T_cam2global = cam.ego2global @ cam.cam2ego
        cam_infos.append({
            'depth_map': depth_map_raw.astype(np.float32),  # нефильтрованная глубина
            'K':         intrinsic,
...
def parse_args():
    p = argparse.ArgumentParser(description="v16: v15 + Bilateral Filter on depth map")
    p.add_argument("--dataroot",   default="/home/max/Phd/Phase1/Sparse4D/data/nuscenes")
    p.add_argument("--version",    default="v1.0-mini")
    p.add_argument("--output_dir", default="./output/pseudo_lidar_nuscenes_v16")
