"""
save_pseudo_lidar_nuscenes_v15s.py

v15s = v15 + per-camera scale correction (DA3METRIC calibration).

DA3METRIC-LARGE систематически занижает глубину на nuScenes.
Поправочные коэффициенты посчитаны на mini-train методом lstsq
(align_scale_shift по GT LiDAR), отдельно для каждой камеры:

    CAM_FRONT       scale=1.4799  (std=0.10 — стабильно)
    CAM_FRONT_LEFT  scale=1.4025  (std=0.25)
    CAM_FRONT_RIGHT scale=1.7106  (std=0.32 — нестабильно)
    CAM_BACK        scale=1.1063  (std=0.07 — очень стабильно)
    CAM_BACK_LEFT   scale=1.3201  (std=0.33)
    CAM_BACK_RIGHT  scale=1.3265  (std=0.11 — стабильно)

shift не применяется: std >> median → перенос на test нестабилен.

Запуск:
    python scripts/v15s/save_pseudo_lidar_nuscenes_v15s.py \\
        --dataroot /home/max/Phd/Phase1/Sparse4D/data/nuscenes \\
        --version v1.0-mini \\
        --output_dir ./output/pseudo_lidar_nuscenes_v15s
"""
...
# Per-camera scale correction (посчитано на nuScenes mini, все 404 sample'а)
# Медианы из lstsq align_scale_shift по GT LiDAR
_CAM_SCALE = {
    "CAM_FRONT":       1.3959,  # std=0.24 — умеренно
    "CAM_FRONT_LEFT":  1.4148,  # std=1.03 — плохо
    "CAM_FRONT_RIGHT": 1.4512,  # std=0.51 — плохо
    "CAM_BACK":        1.0150,  # std=0.18 — лучшая камера
    "CAM_BACK_LEFT":   1.4929,  # std=1.66 — худшая
    "CAM_BACK_RIGHT":  1.4957,  # std=1.26 — вторая худшая
}
...
def build_pseudo_lidar_global(cam_data_dict, estimator, back_proj, range_filter,
    ...
    cam_scales: dict cam_name → float, если None — коррекция не применяется.
    """
    all_points = []
    cam_infos  = []
    ...
    for cam_name, cam in cam_data_dict.items():
        ...
        if cam_scales is not None:
            scale = cam_scales.get(cam_name, 1.0)
            depth_map_raw = (depth_map_raw * scale).astype(np.float32)
...
def parse_args():
    p = argparse.ArgumentParser(description="v15s: v15 + per-camera scale correction")
    p.add_argument("--dataroot",   default="/home/max/Phd/Phase1/Sparse4D/data/nuscenes")
    p.add_argument("--version",    default="v1.0-mini")
    p.add_argument("--output_dir", default="./output/pseudo_lidar_nuscenes_v15s")
    ...
    p.add_argument("--no_scale",  action="store_true",
                   help="Отключить per-camera scale correction")
    p.add_argument("--exclude_scale_cams", nargs="*", default=[],
                   metavar="CAM",
                   help="Камеры, для которых НЕ применять scale (scale=1.0). "
                        "Пример: --exclude_scale_cams CAM_BACK_LEFT CAM_BACK_RIGHT")
...
def main():
    ...
    if args.no_scale:
        ...
        cam_scales = {k: v for k, v in _CAM_SCALE.items()
                      if k not in args.exclude_scale_cams}
    ...
    excluded = args.exclude_scale_cams
    if args.no_scale:
        scale_info = "OFF"
    elif excluded:
        scale_info = f"ON (исключены: {', '.join(excluded)})"
    else:
        scale_info = "ON (все камеры)"
    print(f"Scale corr  : {scale_info}")
    ...
    for scene_idx, scene in enumerate(nusc.scene):
        ...
        while True:
            ...
            # 1. Depth → global (с per-camera scale)
