"""
save_pseudo_lidar_nuscenes_v14.py

v14 = v12 (лучшая версия: Sobel + 32-beam sparsification + RANSAC + jump filter)
     + mean voxelization вместо first-point voxelization

Изменение:
  v12 использовал voxel_downsample(), которая оставляла первую (ближайшую по r)
  точку в каждом вокселе. v14 заменяет это на voxel_downsample_mean(), которая
  усредняет координаты всех точек в вокселе.

  Мотивация (VFMM3D, 2024): усреднение внутри вокселя частично компенсирует
  систематическое смещение depth-модели — случайные ошибки глубины
  на разных пикселях одного вокселя взаимно подавляются.

Запуск:
    python scripts/v14/save_pseudo_lidar_nuscenes_v14.py \\
        --dataroot /home/max/Phd/Phase1/Sparse4D/data/nuscenes \\
        --version v1.0-mini \\
        --output_dir ./output/pseudo_lidar_nuscenes_v14
"""
...
# ─── RANSAC ground plane correction (v12 original — без fix3/fix4) ────────────
...
def parse_args():
    p = argparse.ArgumentParser(description="v14: v12 + mean voxelization")
    p.add_argument("--dataroot",   default="/home/max/Phd/Phase1/Sparse4D/data/nuscenes")
    p.add_argument("--version",    default="v1.0-mini")
    p.add_argument("--output_dir", default="./output/pseudo_lidar_nuscenes_v14")
...
def main():
    ...
    print(f"Voxelization: MEAN (v14 change vs v12 first-point)")
    ...
    for scene_idx, scene in enumerate(nusc.scene):
        ...
        while True:
            ...
            # 1. Depth → global (Sobel edge filter)
            ...
            # 4. Voxel downsampling — MEAN (v14 key change)
            if len(points_sensor) > 0:
                points_sensor = PointCloudFilter.voxel_downsample_mean(
