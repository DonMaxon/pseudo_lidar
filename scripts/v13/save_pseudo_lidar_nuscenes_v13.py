"""
save_pseudo_lidar_nuscenes_v13.py

v13 = v12 с исправлением 4 багов:

Баг 1 (исправлен): Jump filter — неправильный wrap-around по elevation-оси.
  np.roll по axis=0 оборачивал beam 0 ↔ beam 31, хотя они физически далеко.
  Заменено на сдвиг с inf-паддингом (wrap только для azimuth, он цикличен).

Баг 2 (исправлен): Jump filter — коллизии в range image.
  rimg[beam_idx, az_idx] = r записывала последнюю точку в ячейке, а не
  ближайшую. Заменено на np.minimum.at — гарантирует min(r) per cell.

Баг 3 (исправлен): RANSAC Z-диапазон слишком широкий.
  z_max=-0.8 включал точки до 1m выше земли (машины, бордюры).
  Исправлено: z_min=-2.2, z_max=-1.5 (±0.34m вокруг ожидаемого Z=-1.84m).

Баг 4 (исправлен): RANSAC корректировал Z у всех точек включая далёкие.
  Экстраполяция плоскости на 50m+ нестабильна.
  Исправлено: коррекция применяется только к точкам с r_xy ≤ ransac_max_radius.

Запуск:
    python scripts/v13/save_pseudo_lidar_nuscenes_v13.py \\
        --dataroot /home/max/Phd/Phase1/Sparse4D/data/nuscenes \\
        --version v1.0-mini \\
        --output_dir ./output/pseudo_lidar_nuscenes_v13
"""
...
# ─── RANSAC ground plane correction (fix 3 + fix 4) ──────────────────────────

def ransac_ground_plane_correction(points: np.ndarray,
                                    z_min: float = -2.2,
                                    z_max: float = -1.5,
                                    residual_threshold: float = 0.1,
                                    min_ground_pts: int = 100,
                                    max_radius: float = 30.0) -> np.ndarray:
    """
    Корректирует Z-координаты точек по наклону плоскости земли.

    Fix 3: z_min=-2.2, z_max=-1.5 — узкое окно ±0.34m вокруг ожидаемой
    высоты земли (-1.84m в sensor frame), исключает машины и бордюры.

    Fix 4: коррекция применяется только к точкам с r_xy ≤ max_radius (30m).
    На больших расстояниях экстраполяция плоскости нестабильна.

    Args:
        points:            (N, 3) float32 в LiDAR sensor frame
        z_min / z_max:     диапазон Z для кандидатов земли (sensor frame)
        residual_threshold: допуск RANSAC (м)
        min_ground_pts:    минимум точек для запуска RANSAC
        max_radius:        радиус r_xy (м) за пределами которого коррекция не применяется
    """
    if len(points) == 0:
        return points
    ...
    try:
        ...
        # Fix 4: применяем коррекцию только в радиусе max_radius
        r_xy = np.sqrt(points[:, 0] ** 2 + points[:, 1] ** 2)
        apply_mask = r_xy <= max_radius

        corrected = points.copy()
        if apply_mask.any():
            z_pred = ransac.predict(points[apply_mask, :2]).astype(np.float32)
            corrected[apply_mask, 2] += (z_ref - z_pred)
...
# ─── Jump Point Filter (fix 1 + fix 2) ───────────────────────────────────────

def _shift_elevation(arr: np.ndarray, di: int) -> np.ndarray:
    """
    Сдвигает range image по elevation-оси с inf-паддингом (без wrap-around).

    Fix 1: np.roll по axis=0 оборачивал beam 0 ↔ beam N-1 (физически далёкие
    углы). Теперь крайние лучи получают соседа inf (пусто), не друг друга.

    di=-1: смотрим на луч с бо́льшим индексом (выше по elevation)
    di=+1: смотрим на луч с меньшим индексом (ниже по elevation)
    """
    n = arr.shape[0]
    out = np.full_like(arr, np.inf)
    if di == -1:
        out[:n - 1, :] = arr[1:, :]   # строка i ← строка i+1
    elif di == 1:
        out[1:, :] = arr[:n - 1, :]   # строка i ← строка i-1
    else:
        return arr
    return out
...
def jump_point_filter(points: np.ndarray,
    ...
    Fix 1: elevation-ось сдвигается с паддингом (без wrap-around).
    Fix 2: np.minimum.at гарантирует min(r) per cell при коллизиях.
    """
    if len(points) == 0:
        return points
    ...
    # Fix 2: min(r) per cell вместо "последней записи"
    rimg = np.full((n_beams, n_azimuth), np.inf, dtype=np.float32)
    np.minimum.at(rimg, (beam_idx, az_idx), r)

    # Минимальная дальность среди 8 соседей
    # Fix 1: elevation — сдвиг с паддингом; azimuth — roll (цикличен)
    min_neigh = np.full_like(rimg, np.inf)
    for di in (-1, 0, 1):
        elev_shifted = _shift_elevation(rimg, di)   # inf-padding по elevation
        for dj in (-1, 0, 1):
            if di == 0 and dj == 0:
                continue
            shifted = np.roll(elev_shifted, dj, axis=1)   # wrap по azimuth — ок
...
def build_pseudo_lidar_global(cam_data_dict, estimator, back_proj, range_filter,
    ...
    all_points = []
    for _, cam in cam_data_dict.items():
...
def parse_args():
    p = argparse.ArgumentParser(description="v13: v12 с исправлением багов 1-4")
    p.add_argument("--dataroot",   default="/home/max/Phd/Phase1/Sparse4D/data/nuscenes")
    p.add_argument("--version",    default="v1.0-mini")
    p.add_argument("--output_dir", default="./output/pseudo_lidar_nuscenes_v13")
    ...
    # RANSAC (fix 3: новые дефолты z_min/z_max)
    p.add_argument("--no_ransac",        action="store_true")
    p.add_argument("--ransac_z_min",     type=float, default=-2.2,
                   help="Min Z для кандидатов земли в sensor frame (fix3: было -2.5)")
    p.add_argument("--ransac_z_max",     type=float, default=-1.5,
                   help="Max Z для кандидатов земли в sensor frame (fix3: было -0.8)")
    p.add_argument("--ransac_threshold", type=float, default=0.1)
    p.add_argument("--ransac_min_pts",   type=int,   default=100)
    p.add_argument("--ransac_max_radius", type=float, default=30.0,
                   help="Макс. r_xy (м) для применения RANSAC коррекции (fix4)")
    # Jump filter
...
def main():
    ...
    print(f"RANSAC      : {'OFF' if args.no_ransac else f'ON (z=[{args.ransac_z_min},{args.ransac_z_max}], r<={args.ransac_max_radius}m)'}")
    ...
    for scene_idx, scene in enumerate(nusc.scene):
        ...
        while True:
            ...
            # 1. Depth → global (с Sobel edge filter)
            ...
            if not args.no_ransac and len(points_sensor) > 0:
                points_sensor = ransac_ground_plane_correction(
                    ...
                    max_radius=args.ransac_max_radius,
                )
            ...
            # 8. Jump point filter (после sparsification)
