"""
save_pseudo_lidar_nuscenes_v12_1.py

v12.1 = v12 + fix bug 1: elevation wrap-around в jump_point_filter.
np.roll по axis=0 оборачивал beam 0 ↔ beam 31 (физически далёкие углы).
Заменено на _shift_elevation с inf-паддингом.
...
def jump_point_filter(points: np.ndarray,
    ...
    # Fix 1: elevation сдвиг с inf-паддингом (без wrap-around)
    def _shift_elev(arr, di):
        out = np.full_like(arr, np.inf)
        if di == -1:
            out[:arr.shape[0] - 1, :] = arr[1:, :]
        elif di == 1:
            out[1:, :] = arr[:arr.shape[0] - 1, :]
        else:
            return arr
        return out
    ...
    for di in (-1, 0, 1):
        elev_shifted = _shift_elev(rimg, di)
        for dj in (-1, 0, 1):
            if di == 0 and dj == 0:
                continue
            shifted = np.roll(elev_shifted, dj, axis=1)  # azimuth — цикличен
...
def parse_args():
    ...
    p.add_argument("--output_dir", default="./output/pseudo_lidar_nuscenes_v12_1")
